...
// Admin UI static pages base URL.
// For personal use, point this to your own GitHub Pages (so you can customize /admin UI).
...
export default {
    async fetch(request, env, ctx) {
        /* ==== BEGIN ClawBitch additions (debug wrapper) ==== */
        // Hard-fail protection: surface errors instead of generic 1101.
        // If you can't find Cloudflare logs, use:
        //   /__debug?key=<ADMIN>
        // to view the last captured error (stored in KV when available).
        try {
            const url = new URL(request.url);
            // Debug endpoint: show last error captured by try/catch below.
            // This avoids needing the Cloudflare dashboard logs.
            if (url.pathname === '/__debug') {
                const adminKey = env.ADMIN || env.admin || env.PASSWORD || env.password || env.pswd || env.TOKEN || env.KEY || env.UUID || env.uuid;
                const key = url.searchParams.get('key') || '';
                if (!adminKey || key !== adminKey) return new Response('forbidden', { status: 403 });
                const last = (env.KV && typeof env.KV.get === 'function') ? (await env.KV.get('last_error')) : null;
                return new Response(last || 'no last_error recorded (KV not bound yet)', { status: 200, headers: { 'content-type': 'text/plain; charset=utf-8' } });
            }
        /* ==== END ClawBitch additions (debug wrapper) ==== */
        ...
        /* ==== BEGIN ClawBitch additions (error capture) ==== */
        } catch (err) {
            // Capture error for debugging (if KV is available)
            const msg = (err && (err.stack || err.message)) ? String(err.stack || err.message) : String(err);
            try {
                if (env?.KV && typeof env.KV.put === 'function') {
                    await env.KV.put('last_error', `[${new Date().toISOString()}]\n${msg}`);
                }
            } catch (_) {}
            return new Response(`worker error:\n${msg}\n\nTip: open /__debug?key=<ADMIN> to see last_error (if KV bound).`, {
                status: 500,
                headers: { 'content-type': 'text/plain; charset=utf-8', 'cache-control': 'no-store' },
            });
        }
        /* ==== END ClawBitch additions (error capture) ==== */
    }
};
